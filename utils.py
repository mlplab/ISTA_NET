...
def transform_input_mask(input_data, mask, aCH=1):

    bCH, H, W = input_data.shape
    trans_input = input_data.view(-1)
    split_mask = np.zeros((H * W * aCH, H * W * bCH))
    for i in range(H * W * aCH):
        split_mask[i, i::H * W] = 1
    trans_mask = mask * split_mask
    del split_mask

    return input_data, trans_mask
